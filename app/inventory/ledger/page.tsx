...
export default async function StockLedgerPage() {
  // Fetch a larger chunk for the dedicated ledger page
  const { entries: ledger } = await getStockLedger({ pageSize: 100 })
  
  const totalEntries = ledger.length
  const stockIn = ledger.filter(l => l.quantity > 0).reduce((sum, l) => sum + l.quantity, 0)
  const stockOut = ledger.filter(l => l.quantity < 0).reduce((sum, l) => sum + Math.abs(l.quantity), 0)
